    ...
    for k in range(0,feature_amount):
        for i in range(0,layers_by_wall):
            for j in range(0, size_pic_X-feature_size_X):
                ...
                SummResult[k,i,j] = np.sum(local_feature*feature_map[k])
                one_dimension_result[0,0,k*i+j]=SummResult[k,i,j]
...
if __name__ == "__main__":
    ...
    for j in range(0, 30):
        ...
        input.data=capture_feature(face_dataset, feature_map, sample_num, 10)[1]
